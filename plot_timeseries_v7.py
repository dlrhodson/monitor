...
job=sys.argv[2]
webroot=sys.argv[3]
references=sys.argv[4]
QUEUE=sys.argv[5].replace('_',' ')
...
n_jobs=len(canari_names_unique)
...
sbatch='sbatch '+QUEUE+' --time=01:00:00 --array=1-'+str(n_jobs)+' --job-name test -o '+log_dir+'/%A_%a.o -e '+log_dir+'/%A_%a.e '+batch_file
